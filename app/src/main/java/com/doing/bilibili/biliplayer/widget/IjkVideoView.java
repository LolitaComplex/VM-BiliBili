...
public class IjkVideoView extends FrameLayout implements MediaController.MediaPlayerControl {
    ...
    private static final int[] mArrAllAspectRatio = {
    ...
    private ArrayDeque<Integer> mAllRenders = new ArrayDeque<>();
     ...
     * 初始化渲染器
     */
    private void intitRenders() {
        mAllRenders.clear();

        mAllRenders.offer(RENDER_SURFACE_VIEW);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            mAllRenders.offer(RENDER_TEXTURE_VIEW);
        }

        setRender(mAllRenders.pollLast());
    }
    ...
    public void setRender(Integer render) {
        switch (render) {
            case RENDER_SURFACE_VIEW:
                TextureRenderView textureView = new TextureRenderView(getContext());
                if (mMediaPlayer != null) {
                    textureView.getSurfaceHolder().bindToMediaPlayer(mMediaPlayer);
                    textureView.setVideoSize(mMediaPlayer.getVideoWidth(), mMediaPlayer.getVideoHeight());
                    textureView.setVideoSampleAspectRatio(mMediaPlayer.getVideoSarNum(), mMediaPlayer.getVideoSarDen());
                    textureView.setAspectRatio(mArrAllAspectRatio[mCurrentAspectRatioIndex]);
                }
                setRenderView(textureView);
                break;

            case RENDER_TEXTURE_VIEW:
                SurfaceRenderView surfaceView = new SurfaceRenderView(getContext());
                setRenderView(surfaceView);
                break;

            default:
                LogUtils.e("","invalid reader " + render);
                break;
        }
    }
    ...
    private void setRenderView(IRenderView renderView) {
        ...
        renderView.setVideoRotation(mArrAllAspectRatio[mCurrentAspectRatioIndex]);
        renderView.setVideoSize(mVideoWidth, mVideoHeight);
        renderView.setVideoSampleAspectRatio(mVideoSarNum, mVideoSarDen);

        View readerUIView = mRenderView.getView();
        ViewGroup.LayoutParams params = new ViewGroup.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT
        );
        addView(readerUIView, params);

        mRenderView.addRenderCallback(mSHCallback);
        mRenderView.setVideoRotation(mVideoRotationDegree);
    }

    public void setPlayerRotation(int degree) {
    ...
    public void realease(boolean cleartargetstate) {
        if (mMediaPlayer != null) {
            mMediaPlayer.reset();
            mMediaPlayer.release();
            mMediaPlayer = null;
    ...
    private IRenderView.IRenderCallback mSHCallback = new IRenderView.IRenderCallback() {
        ...
        public void onSurfaceChanged(@NonNull IRenderView.ISurfaceHolder holder, int format, int w, int h) {
            ...
            boolean hasValidSize = !mRenderView.isWaitForResize() || (mVideoWidth == w && mVideoHeight == h);
        ...
        public void onPrepared(IMediaPlayer mp) {
            ...
            if (mVideoWidth != 0 && mVideoHeight != 0) {
                ...
                if (mRenderView != null) {
                    ...
                    if (!mRenderView.isWaitForResize() || mSurfaceWidth == mVideoWidth && mSurfaceHeight == mVideoHeight) {
    ...
    public int toggleAspectRatio() {
        ...
        mCurrentAspectRatioIndex %= mArrAllAspectRatio.length;

        if (mRenderView != null)
            mRenderView.setAspectRatio(mArrAllAspectRatio[mCurrentAspectRatioIndex]);
        return mArrAllAspectRatio[mCurrentAspectRatioIndex];
    }
    ...
    public void setAspectRatio(int aspectRatio) {
        for (int i = 0; i < mArrAllAspectRatio.length; i++) {
            if (mArrAllAspectRatio[i] == aspectRatio) {
